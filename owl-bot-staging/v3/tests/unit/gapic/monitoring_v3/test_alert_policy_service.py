...
from google.cloud.monitoring_v3.services.alert_policy_service import AlertPolicyServiceAsyncClient
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    ...
    (AlertPolicyServiceAsyncClient, transports.AlertPolicyServiceGrpcAsyncIOTransport, "grpc_asyncio"),
])
...
@pytest.mark.parametrize("client_class", [
    AlertPolicyServiceClient, AlertPolicyServiceAsyncClient
])
@mock.patch.object(AlertPolicyServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(AlertPolicyServiceClient))
@mock.patch.object(AlertPolicyServiceAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(AlertPolicyServiceAsyncClient))
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (AlertPolicyServiceClient, transports.AlertPolicyServiceGrpcTransport, "grpc", grpc_helpers),
    (AlertPolicyServiceAsyncClient, transports.AlertPolicyServiceGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
])
...
async def test_list_alert_policies_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_alert_policies(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
@pytest.mark.parametrize("client_class,transport_class", [
    (AlertPolicyServiceClient, transports.AlertPolicyServiceGrpcTransport),
    (AlertPolicyServiceAsyncClient, transports.AlertPolicyServiceGrpcAsyncIOTransport),
])
