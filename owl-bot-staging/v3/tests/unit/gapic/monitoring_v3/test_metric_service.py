...
from google.cloud.monitoring_v3.services.metric_service.transports.base import _GOOGLE_AUTH_VERSION
...
def test_metric_service_client_service_account_always_use_jwt(client_class):
...
def test_metric_service_client_service_account_always_use_jwt_true(transport_class, transport_name):
    with mock.patch.object(service_account.Credentials, 'with_always_use_jwt_access', create=True) as use_jwt:
        creds = service_account.Credentials(None, None, None)
...
def test_list_time_series_flattened():
    ...
    with mock.patch.object(
        ...
        assert args[0].interval == common.TimeInterval(end_time=timestamp_pb2.Timestamp(seconds=751))
...
def test_create_time_series_flattened():
    ...
    with mock.patch.object(
        ...
        client.create_time_series(
            name='name_value',
            time_series=[gm_metric.TimeSeries(metric=metric_pb2.Metric(type_='type__value'))],
        )
        ...
        _, args, _ = call.mock_calls[0]
        assert args[0].name == 'name_value'
        assert args[0].time_series == [gm_metric.TimeSeries(metric=metric_pb2.Metric(type_='type__value'))]
