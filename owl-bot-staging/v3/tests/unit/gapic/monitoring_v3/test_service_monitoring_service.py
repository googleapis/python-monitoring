...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (ServiceMonitoringServiceClient, transports.ServiceMonitoringServiceGrpcTransport, "grpc", grpc_helpers),
    (ServiceMonitoringServiceAsyncClient, transports.ServiceMonitoringServiceGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
])
...
async def test_list_service_level_objectives_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_service_level_objectives(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
@pytest.mark.parametrize("client_class,transport_class", [
    (ServiceMonitoringServiceClient, transports.ServiceMonitoringServiceGrpcTransport),
