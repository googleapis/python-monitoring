...
from google.cloud.monitoring_v3.services.query_service.transports.base import _GOOGLE_AUTH_VERSION
...
def test_query_service_client_service_account_always_use_jwt(client_class):
...
def test_query_service_client_service_account_always_use_jwt_true(transport_class, transport_name):
    with mock.patch.object(service_account.Credentials, 'with_always_use_jwt_access', create=True) as use_jwt:
        creds = service_account.Credentials(None, None, None)
...
def test_query_service_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_query_time_series(transport: str = 'grpc', request_type=metric_service.QueryTimeSeriesRequest):
    client = QueryServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
