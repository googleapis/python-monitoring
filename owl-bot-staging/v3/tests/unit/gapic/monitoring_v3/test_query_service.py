...
def test__get_default_mtls_endpoint():
    ...
    assert QueryServiceClient._get_default_mtls_endpoint(api_endpoint) == api_mtls_endpoint
    assert QueryServiceClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert QueryServiceClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert QueryServiceClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
...
def test_query_service_client_service_account_always_use_jwt(transport_class, transport_name):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    ...
    (QueryServiceAsyncClient, transports.QueryServiceGrpcAsyncIOTransport, "grpc_asyncio"),
])
@mock.patch.object(QueryServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(QueryServiceClient))
@mock.patch.object(QueryServiceAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(QueryServiceAsyncClient))
def test_query_service_client_client_options(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    ...
    (QueryServiceAsyncClient, transports.QueryServiceGrpcAsyncIOTransport, "grpc_asyncio", "true"),
    (QueryServiceClient, transports.QueryServiceGrpcTransport, "grpc", "false"),
    (QueryServiceAsyncClient, transports.QueryServiceGrpcAsyncIOTransport, "grpc_asyncio", "false"),
])
@mock.patch.object(QueryServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(QueryServiceClient))
@mock.patch.object(QueryServiceAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(QueryServiceAsyncClient))
@mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "auto"})
def test_query_service_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
@pytest.mark.parametrize("client_class", [
    QueryServiceClient, QueryServiceAsyncClient
])
@mock.patch.object(QueryServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(QueryServiceClient))
@mock.patch.object(QueryServiceAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(QueryServiceAsyncClient))
def test_query_service_client_get_mtls_endpoint_and_cert_source(client_class):
...
def test_query_service_client_client_options_scopes(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (QueryServiceClient, transports.QueryServiceGrpcTransport, "grpc", grpc_helpers),
    (QueryServiceAsyncClient, transports.QueryServiceGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
])
def test_query_service_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_query_service_client_client_options_from_dict():
    with mock.patch('google.cloud.monitoring_v3.services.query_service.transports.QueryServiceGrpcTransport.__init__') as grpc_transport:
...
def test_query_service_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(
        credentials_file="credentials.json"
    )
...
@pytest.mark.asyncio
async def test_query_time_series_async(transport: str = 'grpc_asyncio', request_type=metric_service.QueryTimeSeriesRequest):
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(metric_service.QueryTimeSeriesResponse(
            next_page_token='next_page_token_value',
        ))
...
def test_query_time_series_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, metric.TimeSeriesData)
                   for i in results)
...
async def test_query_time_series_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.query_time_series(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
def test_query_service_base_transport():
    ...
    with mock.patch('google.cloud.monitoring_v3.services.query_service.transports.QueryServiceTransport.__init__') as Transport:
...
def test_query_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.monitoring_v3.services.query_service.transports.QueryServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_query_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.monitoring_v3.services.query_service.transports.QueryServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.QueryServiceTransport()
        adc.assert_called_once()
...
@pytest.mark.parametrize("transport_class", [transports.QueryServiceGrpcTransport, transports.QueryServiceGrpcAsyncIOTransport])
def test_query_service_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
@pytest.mark.parametrize("transport_class", [transports.QueryServiceGrpcTransport, transports.QueryServiceGrpcAsyncIOTransport])
def test_query_service_transport_channel_mtls_with_client_cert_source(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.QueryServiceGrpcTransport, transports.QueryServiceGrpcAsyncIOTransport])
def test_query_service_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
...
def test_client_with_default_client_info():
    ...
    with mock.patch.object(transports.QueryServiceTransport, '_prep_wrapped_messages') as prep:
        client = QueryServiceClient(
            credentials=ga_credentials.AnonymousCredentials(),
...
@pytest.mark.parametrize("client_class,transport_class", [
    (QueryServiceClient, transports.QueryServiceGrpcTransport),
    (QueryServiceAsyncClient, transports.QueryServiceGrpcAsyncIOTransport),
])
