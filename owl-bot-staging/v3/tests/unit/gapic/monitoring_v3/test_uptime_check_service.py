...
@pytest.mark.parametrize("client_class", [
    UptimeCheckServiceClient, UptimeCheckServiceAsyncClient
])
@mock.patch.object(UptimeCheckServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(UptimeCheckServiceClient))
@mock.patch.object(UptimeCheckServiceAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(UptimeCheckServiceAsyncClient))
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (UptimeCheckServiceClient, transports.UptimeCheckServiceGrpcTransport, "grpc", grpc_helpers),
    (UptimeCheckServiceAsyncClient, transports.UptimeCheckServiceGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
])
...
async def test_list_uptime_check_configs_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_uptime_check_configs(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
async def test_list_uptime_check_ips_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_uptime_check_ips(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
@pytest.mark.parametrize("client_class,transport_class", [
    ...
    (UptimeCheckServiceAsyncClient, transports.UptimeCheckServiceGrpcAsyncIOTransport),
])
