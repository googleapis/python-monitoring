...
def test__get_default_mtls_endpoint():
    ...
    assert (
        GroupServiceClient._get_default_mtls_endpoint(api_endpoint) == api_mtls_endpoint
    )
    assert (
        GroupServiceClient._get_default_mtls_endpoint(api_mtls_endpoint)
        == api_mtls_endpoint
    )
    assert (
        GroupServiceClient._get_default_mtls_endpoint(sandbox_endpoint)
...
@mock.patch.object(
    GroupServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(GroupServiceClient)
)
@mock.patch.object(
    GroupServiceAsyncClient,
    "DEFAULT_ENDPOINT",
    modify_default_endpoint(GroupServiceAsyncClient),
)
def test_group_service_client_client_options(
    client_class, transport_class, transport_name
):
...
def test_group_service_client_mtls_env_auto(
    client_class, transport_class, transport_name, use_client_cert_env
):
...
def test_group_service_client_get_mtls_endpoint_and_cert_source(client_class):
...
def test_group_service_client_client_options_credentials_file(
    client_class, transport_class, transport_name, grpc_helpers
):
...
def test_group_service_client_client_options_from_dict():
    with mock.patch(
        "google.cloud.monitoring_v3.services.group_service.transports.GroupServiceGrpcTransport.__init__"
    ) as grpc_transport:
        grpc_transport.return_value = None
        client = GroupServiceClient(client_options={"api_endpoint": "squid.clam.whelk"})
...
def test_group_service_client_create_channel_credentials_file(
    client_class, transport_class, transport_name, grpc_helpers
):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
def test_group_service_base_transport():
    ...
    with mock.patch(
        "google.cloud.monitoring_v3.services.group_service.transports.GroupServiceTransport.__init__"
    ) as Transport:
...
@pytest.mark.parametrize(
    ...
    [
        transports.GroupServiceGrpcTransport,
        transports.GroupServiceGrpcAsyncIOTransport,
        transports.GroupServiceRestTransport,
    ],
)
def test_group_service_transport_auth_gdch_credentials(transport_class):
...
def test_group_service_transport_channel_mtls_with_client_cert_source(transport_class):
    with mock.patch(
...
def test_group_service_transport_channel_mtls_with_adc(transport_class):
    mock_ssl_cred = mock.Mock()
...
@pytest.mark.parametrize(
    ...
    [
        (GroupServiceClient, transports.GroupServiceGrpcTransport),
        (GroupServiceAsyncClient, transports.GroupServiceGrpcAsyncIOTransport),
    ],
)
